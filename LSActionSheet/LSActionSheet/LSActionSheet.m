...
#define kActionSheetCancelButtonTag 2999
#define kActionSheetButtonBaseTag 3000
...
- (instancetype)initWithDelegate:(id)delegate cancelButtonTitle:(NSString *)cancelButtonTitle itemsButtonTitle:(NSArray *)itemsButtonTitle {
    ...
    if (self = [super init]) {
        ...
        for (int i = 0; i < itemsButtonTitle.count; i ++) {
            [self createBtnWithTitle:itemsButtonTitle[i] backgroundColor:[UIColor whiteColor] index:kActionSheetButtonBaseTag + i];
        }
        ...
        cancelBtn.tag = kActionSheetCancelButtonTag;
...
- (void)createBtnWithTitle:(NSString *)title backgroundColor:(UIColor *)backgroundColor index:(NSInteger)index {
    ...
    if (index - kActionSheetButtonBaseTag == 0) {
...
- (void)show {
    ...
    // actionSheet
    CGFloat actionSheetH = (self.btnArray.count + 1) * kActionSheetButtonHeight + 5;
    self.actionSheet.frame = CGRectMake(0, self.height, SCREEN_WIDTH, actionSheetH);
    
    // cancelButton
    self.cancelButton.frame = CGRectMake(0, actionSheetH - kActionSheetButtonHeight, SCREEN_WIDTH, kActionSheetButtonHeight);
    
    CGFloat btnH = kActionSheetButtonHeight;
...
- (void)actionSheetClickedButtonAtIndex:(UIButton *)sender {
    ...
    if (sender.tag == kActionSheetCancelButtonTag) {
        [self dismiss];
    } else {
        if (sender.tag - kActionSheetButtonBaseTag == 0) {
            
        } else {
            if (self.delegate && [self.delegate respondsToSelector:@selector(LSActionSheet:didClickButtonAtIndex:)]) {
                [self.delegate LSActionSheet:self didClickButtonAtIndex:sender.tag - kActionSheetButtonBaseTag];
            }
            [self dismiss];
        }
    }
